...
ui <- fluidPage(titlePanel("SPFL xG Game Maps"),
...
server <- function(input, output) {
  ...
  output$homeList <- renderUI({
    ...
    homeTeams <- dplyr::filter(
      ...
      pull() %>%
      as.character()
    selectInput("home", "Home team", sort(c("",unique(homeTeams))))
  })
  ...
  output$awayList <- renderUI({
    ...
    awayTeams <- dplyr::filter(
      ...
      pull() %>%
      as.character()
    selectInput("away", "Away team", sort(c("",unique(awayTeams))), selected = "")
  })
